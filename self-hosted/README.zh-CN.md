...
当 AI 在电脑上的浏览器操作遇到必须由人完成的登录时，你可以用手机接管**这一张 Chrome 页面**完成登录。同一条私密限时链接会直接返回给当前 AI 会话，也会通过带有 ToolArks 品牌的邮件发送，即使你没有盯着会话也能收到通知。
...
命令会拒绝“找不到”或“匹配多个”的情况，然后启动本地网关、命名 Tunnel 或 Quick Tunnel，检查公网入口，并用两种方式交付同一条完整临时链接：在终端输出供 AI 直接回复到当前会话，同时发送邮件到 `REMOTE_RELAY_NOTIFY_TO`。正常流程不会让你把密码或验证码粘贴到聊天里。
...
2. 在真实交接前先验证邮件发送；
3. 只选择一张准确的登录标签页；
4. 在当前会话返回私密手机链接，同时发送带 ToolArks 品牌的邮件通知；
5. 验证原页面状态并停止 Relay。
...
链接片段里有随机控制令牌。收到完整链接的人可以在它过期或 Relay 停止前控制这张 Chrome 页面。直接交付会让本地终端和当前 AI 会话服务处理这条完整链接，它们可能按各自的历史与日志政策保留链接。它只共享一张准确的标签页，不共享整个桌面。不要转发、公开发布链接，也不要把 Chrome 调试端口暴露到局域网或公网。请使用专用 Chrome 配置和较短会话。
